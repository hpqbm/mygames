...
var Ball = function(position, speed) {
    ...
    this.resetVelocity = function() {
        this.theta = Math.random()*360; //random(0, 360);
        this.velocity = new p5.Vector(this.speed*Math.cos(this.theta), -this.speed*Math.sin(this.theta));
        player2Y = height/2;
    };
    this.resetVelocity();
    ...
    this.collideWithPaddle = function(x, y) {
        ...
        this.position.x + this.radius > x - PADDLE_WIDTH/2) {
            ...
            PADDLE_HEIGHT/2 + this.radius) {
                ...
                this.velocity.mult(new p5.Vector(-1, 1));
            }
        }
    };
...
ball = new Ball(new p5.Vector(width/2, height/2));
